...
export default function CalendarClient({ token }: CalendarClientProps) {
  ...
  // Scroll lock when mobile drawer is open (only on mobile viewports < 1024px)
  useEffect(() => {
      ...
      <div className="space-y-6">
        ...
        {bookings === undefined ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                      ...
                      {gridCells.map((cell, idx) => {
                        if (!cell) {
                          return (
                            <div
                              ...
                              className="min-h-[2rem] sm:min-h-[8rem] bg-transparent border border-transparent pointer-events-none"
                            />
                          );
                        }
                        ...
                        return (
                          <div
                            ...
                            className={`min-h-[2rem] sm:min-h-[8rem] p-2.5 rounded-2xl bg-white flex flex-col justify-between transition cursor-pointer select-none border shadow-3xs hover:shadow-2xs group ${isSelected
                              ? "border-transparent ring-2 ring-brand-primary bg-brand-soft"
                              : isToday
            ...
            {/* Desktop Right side: Selected Day Detail Sidebar Cards */}
            <div className="hidden lg:block lg:sticky lg:top-20 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs p-5 space-y-3">
                <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                  <Filter className="w-4 h-4 text-green-800" />
                  <span>Filtre par type d'action :</span>
                </div>
                <select
                  value={typeFilter}
                  onChange={(e) => setTypeFilter(e.target.value as any)}
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-primary cursor-pointer"
                >
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs p-5 space-y-4">
                ...
                {/* Top Section with selected period */}
                <div className="flex justify-between items-start gap-2">
                  <div className="space-y-1">
                    <span className="text-[10px] font-extrabold uppercase text-green-800 tracking-wider">
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider pb-1">
                  <CalendarDays className="w-4 h-4 text-green-800" />
            ...
            {isDrawerOpen && (
              <div
                className="fixed inset-0 bg-black/40 backdrop-blur-3xs z-40 lg:hidden animate-in fade-in duration-200"
                onClick={() => setIsDrawerOpen(false)}
              />
            )}
            <div
              className={`fixed bottom-0 left-0 right-0 z-50 bg-stone-50 rounded-t-3xl border-t border-slate-200 shadow-2xl px-5 pt-3 pb-8 lg:hidden transition-transform duration-300 ease-out max-h-[85vh] flex flex-col gap-4 ${
                isDrawerOpen ? "translate-y-0" : "translate-y-full"
              }`}
            >
              {/* Slide Handler (visual indicator) */}
              <div className="flex flex-col items-center gap-1 cursor-pointer shrink-0 pb-1" onClick={() => setIsDrawerOpen(false)}>
              ...
              <div className="flex flex-col gap-4 overflow-y-auto pr-1">
                {/* Mobile Filter select inside Drawer */}
                <div className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3 shrink-0">
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                    <Filter className="w-4 h-4 text-green-800" />
                    <span>Filtre par type d'action :</span>
                  </div>
                ...
                {/* Mobile Selected Day Detail List inside Drawer */}
                <div className="bg-white rounded-2xl border border-slate-200 p-4 space-y-4 flex-grow">
                  <div className="flex justify-between items-start gap-2">
                    <div className="space-y-1">
                      <span className="text-[10px] font-extrabold uppercase text-green-800 tracking-wider">
                        Période Sélectionnée
                  ...
                  <div className="space-y-4">
                    {selectedDateEvents.length === 0 ? (
                      ...
                      selectedDateEvents.map((ev, eIdx) => {
                        ...
                        return (
                          <div
                            key={`mob-${booking._id}-${ev.type}-${eIdx}`}
